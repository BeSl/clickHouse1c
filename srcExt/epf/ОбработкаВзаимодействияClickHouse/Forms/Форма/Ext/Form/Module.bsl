...
Функция ПараметрыПодключения()
	...
	ПараметрыПодключения.Вставить("АдресСервера", прм_АдресСервера);
	ПараметрыПодключения.Вставить("Порт", прм_Порт);
	ПараметрыПодключения.Вставить("Логин", прм_Логин);
	ПараметрыПодключения.Вставить("Пароль", прм_Пароль);	
...
Процедура СтруктураБазСервераПриАктивизацииСтроки(Элемент)
	...
	Если Элемент.ТекущиеДанные.Картинка = БиблиотекаКартинок.ВнешнийИсточникДанныхКуб Тогда	
		НоваяВидимостьЭлементов("кмСоздатьБазуДанных,кмСоздатьТаблицу","кмЗапросКТаблице,кмВставкаДанных");
	ИначеЕсли Элемент.ТекущиеДанные.Картинка = БиблиотекаКартинок.ВнешнийИсточникДанныхТаблица Тогда
		НоваяВидимостьЭлементов("кмЗапросКТаблице,кмВставкаДанных","кмСоздатьБазуДанных,кмСоздатьТаблицу");
	Иначе
		НоваяВидимостьЭлементов(,"кмЗапросКТаблице,кмСоздатьБазуДанных,кмСоздатьТаблицу,кмВставкаДанных");
	КонецЕсли;
...
Функция НовыйЗапрос()
	...
	кхЗапрос = Новый Структура;
	
	кхЗапрос.Вставить("сВыбрать", Новый Структура("Первые, Различные"));
	кхЗапрос.Вставить("сИЗ",  Новый Структура("Таблицы"));//массив????
	кхЗапрос.Вставить("сГДЕ",  Новый Структура("ОТБОРЫ"));
	кхЗапрос.Вставить("сФормат",  Новый Структура("джсон, хмл и тд"));
